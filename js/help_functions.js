...
function spawn_tile() {
	...
	var tile = {val: Math.random() < 0.9 ? 2 : 4, is_merged: false};
	// add to board array
...
keypress_functions[13] = function start_game() {
	// resets variables
	board.tiles = Array.apply(null,Array(16)).map(Number.prototype.valueOf,0);
	board.score = 0;
	board.is_legal = [];
	board.is_running = false;
	spawn_tile();
...
///// moving functions, moves (and merges) all possible tiles /////
// up (w) = 119
keypress_functions[119] = function move_up() {
	for (var k = 1; k <= 3; k++) {
  		for (var i = 4; i < 16; i++) {
  			if(board.tiles[i] != 0 && !board.tiles[i].is_merged) {
  				move_or_merge(i,-4)
       		}
  		}
	}
...
keypress_functions[97] = function move_left() {
	for (var i = 1; i < 16; i++) {
		if(i%4 != 0 && board.tiles[i] != 0 && !board.tiles[i].is_merged) {
			move_or_merge(i,-1)
       	}
	}	
}
...
keypress_functions[100] = function move_right() {
	for (var i = 14; i > -1; i--) {
		if(i%4 != 3 && board.tiles[i] != 0 && !board.tiles[i].is_merged) {
			move_or_merge(i,1)
		}
	}	
}